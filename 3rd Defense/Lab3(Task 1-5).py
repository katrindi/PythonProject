...
# 5.1 Select items that have related components
# (returns all rows from the Item table that have matching rows in the Component table)
print("-------ITEMS WITH RELATED COMPONENTS--------")
# create a dictionary to store items with related components
related_items = {}
...
for component in session.query(Component).all():
    ...
    for item in items_with_component:
        if component.name in related_items:
            related_items[component.name].append(item.name)
        else:
            related_items[component.name] = [item.name]

# print the items with related components
for component, items in related_items.items():
    if len(items) > 1:
